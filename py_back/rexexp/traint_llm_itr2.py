# train_characteristics_model_v3.py
# Обучает мультилейбл-модель для предсказания ключей характеристик.
# Улучшения:
#  - агрессивная фильтрация мусора (generic "Вид", стандарты)
#  - таргеты = все нормализованные ключи товара (а не только пересечение с онтологией)
#  - class weights (pos_weight) по частоте меток
#  - категория зашивается в текст: [CAT_{id_категории}] ...
...
NORMALIZATION_MAP = {
    # более специфичный ключ ставим раньше, чтобы не наехал общий
...
def normalize_key(key: str) -> str:
    k = key.lower().strip()
    # сперва пробегаем по словарю нормализации
...
# ============================= ОНТОЛОГИЯ И ПРОСТРАНСТВО КЛЮЧЕЙ =============================
...
class CharacteristicsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # парсим реальные key:value в товаре
        ...
        # ТАРГЕТЫ = все item_keys, которые есть в глобальном словаре all_keys
        ...
        return {
            "category_id": cat_id,
            "text": base_text,  # без [CAT_], добавим в collate_fn
            "labels": y,
        }
...
for i in tqdm(range(len(dataset)), desc="Скан датасета"):
    y = dataset[i]["labels"]
    pos_counts += y
...
def collate_fn(batch):
    # текст = [CAT_{id_категории}] + base_text
